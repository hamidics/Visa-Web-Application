		...
		<form action="karvans.php" method="post" accept-charset="utf-8" id="myform"  class="formular">
			...
			<div id="tabs" align="right" style='font-family:tahoma' >
				...
				<?php 
					...
					-->
					<input type="submit" class="submit"  name="viewallkarvans" value="نمایش همه کاروانها" />
					</br>
					<?php 
				...
				if($_SESSION['user_type']=="user"){
					...
					$querycont="and  (laboratory='0000-00-00' || laboratory='')";
				$sql="select `mosques`.rahgiri_code, `karvan`.id as id, `karvan`.code, `karvan`.created_by,  `karvan`.mosque_id, recipe_date  from mosques, karvan where `mosques`.id=`karvan`.mosque_id and  `karvan`.created_by='$_SESSION[user_id]' and `mosques`.name LIKE '%$searchval%' and `karvan`.status='new' and report='0000-00-00' $querycont order by id desc";
