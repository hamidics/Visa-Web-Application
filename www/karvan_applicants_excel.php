...
<?php	
	...
	$styleThinBlackBorderOutline = array(
		...
		$objPHPExcel->getActiveSheet()->setCellValue('D1', "فهرست اسامی افراد   ".$mosquename);
	...
	$sql="select applicant_id,last_address_id,karvan_id from karvan_applicants_getinfo where karvan_id='$karvanid'";
				...
				$ro=mysql_fetch_array($rs);
					...
					//Printing into excel file
						...
						$objPHPExcel->getActiveSheet()->setCellValue('K'.$i, $j);
						$objPHPExcel->getActiveSheet()->setCellValue('J'.$i, $ro['name']);
						$objPHPExcel->getActiveSheet()->setCellValue('I'.$i, $ro['father_name']);
						$objPHPExcel->getActiveSheet()->setCellValue('H'.$i, $ro['birth_date']);
						$objPHPExcel->getActiveSheet()->setCellValue('G'.$i, $ro['birth_place']);
