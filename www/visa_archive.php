...
?>
		...
		<form method="post" action="visa_archive.php" accept-charset="utf-8" id="myform"  class="formular">
			<fieldset>
				...
				<?php
				    if(isset($_POST['search'])){
					    ...
					    if($_POST['search_type'] == "name"){
						   ...
						   $sql="Select id, created_at, first_name, last_name, passport_num, passport_kind, status from visa_applicants where first_name LIKE '%".$_POST['search_data']."%' and type='old' $formstatus order by id desc ";
				           ...
				           while($row=mysql_fetch_array($res)){
							  ...
							  <a href='visa_archive_edit.php?edit_id=$id'><img src='img/edit.png' width='15px' height='15'/></a> 
						...
						elseif($_POST['search_type'] == "passport_num"){
						   $sql="Select id, created_at, first_name, last_name, passport_num, passport_kind, status from visa_applicants where passport_num LIKE '$_POST[search_data]' and type='old'  $formstatus order by id desc";
				           $res=mysql_query($sql) or die(mysql_error());
				           while($row=mysql_fetch_array($res)){
						      $number++;
						      $id = $row['id'];
						  ...
						  $sql="Select id, created_at, first_name, last_name, passport_num, passport_kind, status from visa_applicants where type='old' $formstatus order by id desc";
				          $res=mysql_query($sql) or die(mysql_error());
				           while($row=mysql_fetch_array($res)){
						      $number++;
						      $id = $row['id'];
