...
function checkkafil(val){
	if(val=="none"){
		...
		<form action="visa_karvan_appnew.php?kid=<?php echo $_GET['kid'];?>" method="post" accept-charset="utf-8" id="myform"  enctype="multipart/form-data" class="formular">
			<fieldset>
...
<tr align="right">
				...
				<?php
				    if(isset($_POST['save_applicant'])){
						...
						$sq="select limits from visa_karvan where id='$_GET[kid]'";
						$rslimit=mysql_query($sq) or die (mysql_error());
						$rwlimit=mysql_fetch_array($rslimit);
						$sql="select count(id) from visa_applicants where source_id='$_GET[kid]'";
						$rscount=mysql_query($sql) or die (mysql_error());
						$rwcount=mysql_fetch_array($rscount);
						if($rwcount['count(id)']<$rwlimit['limits']){
						
						 $filename=$_FILES['app_photo']['name'];
						 $file_ext = substr($filename, strripos($filename, '.'));
						 if($file_ext==".jpg" || $file_ext==".JPG"){
						 $file_ext=".jpg";
					    $sql="Insert into  visa_applicants(created_at, first_name, last_name, father_name, birth_date, birth_place, marital_status, passport_kind, passport_num, passport_issue_date, passport_issue_place, job, education, add_in_iran, phone_in_iran, add_in_afghanistan, phone_in_afghanistan, job_add_in_afghanistan, job_phone_in_afghanistan, type, kafil_id, kafil_relation, source_id) values('$_POST[app_created_at]', '$_POST[app_first_name]', '$_POST[app_last_name]', '$_POST[app_father_name]', '$_POST[app_birth_date]', '$_POST[app_birth_place]', '$_POST[app_marital_status]', '$_POST[passport_kind]', '$_POST[passport_num]', '$_POST[passport_issue_date]', '$_POST[passport_issue_place]', '$_POST[app_job]', '$_POST[app_education]', '$_POST[add_in_iran]', '$_POST[phone_in_iran]', '$_POST[add_in_afghanistan]', '$_POST[phone_in_afghanistan]', '$_POST[job_add_in_afghanistan]', '$_POST[job_phone_in_afghanistan]', 'new', '$_POST[kafilname]', '$_POST[relationship]', '$_GET[kid]')";
					   ...
					   $image->save($destination);
						 if($res && $res1 && $rslimit && $rscount){
						 mysql_query("commit;");
						 }else{
						 mysql_query("rollback;");
						 }
						 header("location: visa_karvan_appnew.php?kid=$_GET[kid]");
						die();
						}else{
				...
				<tr>
					...
					<option value="">انتخاب</option>
					<?php
					    $sql="select id, passport_num, first_name, last_name from  visa_applicants  where type='new' and source_id='$_GET[kid]' order by id desc";
						$res=mysql_query($sql);
				...
				<?php
				    if(isset($_POST['save_acco'])){
				       ...
				       header("location: visa_karvan_appnew.php?kid=$_GET[kid]#tabs-2");
					   die();
					}
					else{
				 ...
				 <?php
				    if(isset($_POST['save_visa'])){
						...
						if($rs1 && $rs2){
				         ...
				         header("location: visa_karvan_appnew.php?kid=$_GET[kid]#tabs-3");
						 die();
					}
				?>
 		</fieldset>
