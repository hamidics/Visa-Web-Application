...
require_once '../../library/pdate.php';
			...
			$objPHPExcel = $objReader->load("../excel_templates/border.xls");
			
	$viewid=$_GET['karvanid'];
	$today=pdate("Y-m-d");
	$sql="select code,mosque_id, visa_price, recipe_date, border, border_time from karvan where id='$viewid'";
	...
	//echo $responsibleid;
						...
						$objPHPExcel->getActiveSheet()->setCellValue('B19', $ro['border']);
						$objPHPExcel->getActiveSheet()->setCellValue('B20', $row['phone_num']);
						
						$objPHPExcel->getActiveSheet()->setCellValue('B21', $ro['border_time']);
