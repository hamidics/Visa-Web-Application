...
require_once '../../library/pdate.php';
			...
			$objPHPExcel = $objReader->load("../excel_templates/car.xls");
			
	$viewid=$_GET['karvanid'];
	$today=pdate("Y-m-d");
	$sql="select code,mosque_id, visa_price, recipe_date, lab_time, laboratory, car, driver_name, car_name from karvan where id='$viewid'";
	...
	//echo $responsibleid;
						...
						$objPHPExcel->getActiveSheet()->setCellValue('B12', $roo['name']);
						$objPHPExcel->getActiveSheet()->setCellValue('B13', $row['name']." ".$row['f_name']);
						$objPHPExcel->getActiveSheet()->setCellValue('B14', $row3['count(id)']);
						$objPHPExcel->getActiveSheet()->setCellValue('B15', $ro['car']);
						$objPHPExcel->getActiveSheet()->setCellValue('B16', $ro['driver_name']);
						$objPHPExcel->getActiveSheet()->setCellValue('B17', $ro['car_name']);
			...
			header('Content-Disposition: attachment;filename="car.xls"');
