...
require_once '../../library/pdate.php';
			...
			$objPHPExcel = $objReader->load("../excel_templates/hamahangi.xls");
			
	$viewid=$_GET['karvanid'];
	
	$sql="select code,mosque_id, visa_price, recipe_date, border, destination from karvan where id='$viewid'";
	...
	$today=pdate("Y-m-d");
						...
						$objPHPExcel->getActiveSheet()->setCellValue('B15', $ro['border']);
						$objPHPExcel->getActiveSheet()->setCellValue('B16', $row['phone_num']);
						
						$objPHPExcel->getActiveSheet()->setCellValue('B17', $ro['destination']);
			...
			header('Content-Disposition: attachment;filename="hamahangi.xls"');
